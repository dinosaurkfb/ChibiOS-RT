...
static void write_byte_exe(void) {
  ...
  for (i = 0; i < MIN(test_bytes, EEPROM_SIZE); i++, addr++) {
    test_assert(i, RDY_OK == mc24lc0x_write_byte(&I2CD1, addr, txbuf[i]), "write byte failed.");
  }
}
...
static void random_read_exe(void) {
  ...
  uint32_t test_bytes = TEST_BYTES;
  for (i = 0; i < MIN(test_bytes, EEPROM_SIZE); i++, addr++) {
    test_assert(i, RDY_OK == mc24lc0x_random_read(&I2CD1, addr, &rxbuf[i]), "random_read failed.");
    /* if (i == 0) { */
    /*   I2C_Dump(); */
    /* } */
  }
...
static void cur_read_exe(void) {
  ...
  mc24lc0x_random_read(&I2CD1, EEPROM_SIZE - 1, &rbyte);

  for (i = 0; i < MIN(test_bytes, EEPROM_SIZE); i++, addr++) {
    test_assert(i, RDY_OK == mc24lc0x_cur_read(&I2CD1, &rxbuf[i]), "current_read failed.");
  }
}
